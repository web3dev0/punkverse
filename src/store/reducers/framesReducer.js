...
const reorderFrame = (frames, action) => {
  ...
  const insertPosition =
    destinationIndex + (targetIsBefore ? 1 : 0);
...
const deleteFrame = (frames, action) => {
  ...
  const reduceFrameIndex =
    activeIndex >= frameId && activeIndex > 0;
  return frames.merge(
    {
...
const setFrames = (frames, action) => {
  ...
  const activeIndex = frames.get('activeIndex') ?? 0;
  const frameList = frames
    .get('list')
    .set(activeIndex, action.frames.get(0));
  return fromJS({
    list: frameList.toJS(),
    columns,
    rows,
