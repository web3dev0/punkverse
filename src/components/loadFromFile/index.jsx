...
const LoadFromFile = props => {
  ...
  const onCreateProject = () => {
    ...
    if (
      ...
      const columns = CanvasByPixelSize.width;
      const rows = CanvasByPixelSize.height;

      const frames = generateFrames(context, pixelSize, {
        columns,
        rows
      });

      actions.setDrawing(frames, [], pixelSize, columns, rows);
      close();
    }
  };
    ...
    <Container>
      ...
      <LoadedImageContainer imageLoaded={imageLoaded}>
        ...
        <Button
          ...
          disabled={
            validationError.widthError ||
            validationError.heightError
          }
        >
          CREATE PROJECT
        </Button>
